...
const DOMAINS = {
    'software-dev': { name: 'Software Developer', icon: '≡ƒÆ╗', keywords: 'python java c++ software architecture algorithms coding back end express node' },
    'web-dev': { name: 'Web Developer', icon: '≡ƒîÉ', keywords: 'html css javascript react node backend frontend fullstack website mern mean' },
    'mobile-dev': { name: 'Mobile App Developer', icon: '≡ƒô▒', keywords: 'ios android react native flutter swift kotlin mobile app application' },
    'cyber-analyst': { name: 'Cybersecurity Analyst', icon: '≡ƒ¢í∩╕Å', keywords: 'security defense hacking network pentest ethical hacker firewall ids ips' },
    'cloud-eng': { name: 'Cloud Engineer', icon: 'Γÿü∩╕Å', keywords: 'aws azure gcp devops infrastructure serverless cloud scaling s3 ec2' },
    'devops-eng': { name: 'DevOps Engineer', icon: 'ΓÖ╛∩╕Å', keywords: 'cicd docker kubernetes terraform automation pipeline ansible' },
    'data-analyst': { name: 'Data Analyst', icon: '≡ƒôè', keywords: 'excel sql tableau powerbi analytics visualization bi reporting pandas' },
    'data-scientist': { name: 'Data Scientist', icon: '≡ƒº¬', keywords: 'machine learning statistics python r analytics modeling big data scikit' },
    'business-analyst': { name: 'Business Analyst', icon: '≡ƒÆ╝', keywords: 'requirements process management strategy scrum product owner agile' },
    'ai-engineer': { name: 'AI Engineer', icon: '≡ƒºá', keywords: 'openai chatbot llm neural networks generative ai nlp gpt vertex langchain' },
    'ml-engineer': { name: 'Machine Learning Engineer', icon: '≡ƒñû', keywords: 'tensorflow pytorch deep learning models deployment training' }
};
...
function renderDomainCards(grid, domains) {
    grid.innerHTML = domains.map(([id, data]) => `
        <div class="card domain-card fade-in" onclick="selectDomain('${id}')">
            ...
            <button class="btn btn-ghost btn-start orbitron" style="font-size: 0.7rem; width: 100%; justify-content: center;">START MISSION ΓåÆ</button>
        </div>
    `).join('');
}
...
async function renderDashboardActivePaths() {
    ...
    list.innerHTML = Object.entries(DOMAINS).slice(0, 5).map(([id, data]) => `
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px; background: white; border-radius: 8px;">
            ...
            <button onclick="selectDomain('${id}')" style="background: var(--light-blue); border: none; padding: 4px 12px; border-radius: 4px; font-size: 0.65rem; color: var(--primary-blue); cursor: pointer; font-family: 'Orbitron';">RESUME ΓåÆ</button>
        </div>
    `).join('');
}
    ...
    try {
        ...
        container.innerHTML = stages.map((stage, sIdx) => {
            ...
            const linksHTML = (stage.articles || []).concat(stage.blogs || []).map(link =>
                `<a href="${link.url}" target="_blank" style="font-size: 0.7rem; color: var(--primary-blue); text-decoration: underline; margin-right: 15px;">≡ƒöù ${link.name}</a>`
            ).join('') || '';
            ...
            const topicsHTML = stage.topics.map((topic, tIdx) => {
                ...
                if (isCompleted) {
                    ...
                    statusLabel = 'COMPLETED Γ£ô';
                } else if (!nextFound) {
                    blinkClass = 'blink-next';
                    statusLabel = 'IN PROGRESS ΓÜí';
                    nextFound = true;
                }
                ...
                return `
                    <div class="${classes} ${blinkClass}" style="padding: 20px; margin-bottom: 15px; border-left: 4px solid var(--primary-blue);">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <a href="${youtubeLink}" target="_blank" onclick="markComplete('${domainId}', ${stageId}, ${outcomeId})" class="btn btn-ghost" style="padding: 5px 12px; font-size: 0.6rem;">WATCH VIDEO ΓåÆ</a>
                        </div>
                    </div>
                `;
            }).join('');
            ...
            if (isAssessmentDone) {
                ...
                assessmentLabel = 'MASTERY VALIDATED Γ£ô';
...
function renderQuizStep() {
    ...
    container.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            ...
            ${currentQuiz.currentIndex > 0 ? `<button onclick="prevQuizStep()" class="btn btn-ghost" style="padding: 4px 10px; font-size: 0.6rem;">ΓåÉ BACK</button>` : '<span></span>'}
        </div>
    ...
    if (currentQuiz.answers[currentQuiz.currentIndex] !== null) {
        ...
        container.innerHTML += `<button onclick="nextQuizStep()" class="btn btn-primary" style="width: 100%; margin-top: 20px; justify-content: center;">${currentQuiz.currentIndex === currentQuiz.questions.length - 1 ? 'FINISH' : 'NEXT ΓåÆ'}</button>`;
    }
}
        ...
        if (window.location.pathname.includes('roadmap.html')) {
            ...
            if (container && !document.getElementById('cert-alert')) {
                ...
                alertDiv.innerHTML = `
                    ...
                    <button onclick="window.location.href='profile.html'" class="btn" style="background: white; color: var(--navy-blue);">VIEW CERTIFICATE ΓåÆ</button>
                `;
        ...
        certGrid.innerHTML = certificates.map(id => {
            ...
            return `
                <div class="card fade-in" style="padding: 30px; text-align: center; border: 2px solid var(--primary-blue); background: white;">
                    <div style="font-size: 3.5rem; margin-bottom: 20px;">≡ƒÄû∩╕Å</div>
...
function renderResources() {
    ...
    const resources = [
        { name: `${domainName} Mastery`, provider: 'Edureka', url: `https://www.edureka.co/search/${encodeURIComponent(domainName)}`, icon: '≡ƒÆÄ' },
        { name: `${domainName} Bootcamp`, provider: 'Udemy', url: `https://www.udemy.com/courses/search/?q=${encodeURIComponent(domainName)}`, icon: '≡ƒÄô' },
        { name: `${domainName} Essential Skills`, provider: 'YouTube', url: `https://www.youtube.com/results?search_query=${encodeURIComponent(domainName)}+tutorial`, icon: '≡ƒô║' },
        { name: `Advanced ${domainName} Guide`, provider: 'Medium', url: `https://medium.com/search?q=${encodeURIComponent(domainName)}`, icon: '≡ƒô¥' }
    ];
    ...
    container.innerHTML = resources.map(res => `
        <div class="card lib-card scroll-reveal" style="padding: 25px; border-left: 4px solid var(--primary-blue);">
            ...
            <a href="${res.url}" target="_blank" class="btn btn-primary" style="padding: 10px 15px; font-size: 0.65rem; width: 100%; justify-content: center;">START MISSION ΓåÆ</a>
        </div>
    `).join('');
}
